...
if (!$stmtUser) {
    error_log('Error: MySQL prepare failed for user query - ' . $mysqli->error);
...
if (!$stmtUser->execute()) {
    error_log('Error: MySQL execute failed for user query - ' . $stmtUser->error);
    header('HTTP/1.0 500 Internal Server Error');
    echo json_encode(['error' => true, 'message' => 'Internal Server Error']);
    exit();
}
...
");
if (!$stmtTasks) {
    error_log('Error: MySQL prepare failed for tasks query - ' . $mysqli->error);
    header('HTTP/1.0 500 Internal Server Error');
    echo json_encode(['error' => true, 'message' => 'Internal Server Error']);
    exit();
}
...
if (!$stmtTasks->execute()) {
    error_log('Error: MySQL execute failed for tasks query - ' . $stmtTasks->error);
    header('HTTP/1.0 500 Internal Server Error');
    echo json_encode(['error' => true, 'message' => 'Internal Server Error']);
    exit();
}
